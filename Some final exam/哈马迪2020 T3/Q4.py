...
4321, 12331, 12421, 13321]
    ...
    reslts = []
    reslts_set = set()
    def find(current, index, current_sum):
        if current_sum == sum_of_digits:
            reslt = int(current)
            if reslt not in reslts_set:
                reslts.append(reslt)
                reslts_set.add(reslt)
        if current_sum > sum_of_digits or index > len(digit):
            return
        if index < len(digit):
            find(current + digit[index], index+1, current_sum+int(digit[index]))

        find(current,index+1,current_sum)

    find('',0,0)
    return sorted(reslts)
